...
function checkIfFinished()
{
    ...
    {
        alert("השלמתם את כל הפאזלים! עתה, חשבו את סכום הספרות המוסתרות על חלקי הפאזל,ומצאו את הפיסת קוד הסודית אשר כתובה באנגלית, ואז לחצו על כפתור ה\"סיום\" בפינת המסך.");
...
function done()
{
    var qTxt = "מה הוא סכום המספרים אשר יצא לכם יחד עם המילה באנגלית? צריך להכניס את הנתונים בצורה הבאה:\n";
    var ext = "משמאל לימין קודם המילה באנגלית ולאחר מכן המספר שיצא ללא רווחים ביניהם";
    qTxt += ext;
    var errTxt = "טעות. בדקו את עצמכם ולחצו שוב על הכפתור";
    var correctTxt = "התשובה נכונה! כל הכבוד!";
    
    if (prompt(qTxt).toUpperCase() == "VOID173")
    {
        
        var audio = new Audio('resources/audio/right-answer.mp3');
        audio.play();
        alert("כל הכבוד, סיימת את הפאזל, הפאזל הוציא את הקוד\n קוד");
    }
    else
    {
        alert(errTxt);
